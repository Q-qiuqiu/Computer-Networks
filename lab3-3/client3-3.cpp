...
u_short check(u_short* head, int size)
{
    ...
    //u_short* buf = new u_short[size + 1];;//动态分配字符串变量
    memset(buf, 0, size + 1);//数组清空
    memcpy(buf, head, size);//数组赋值
    u_long checkSum = 0;
    u_short* originalBuf = buf;  // 保存原始指针
    while (count--) {
...
void newbag2(Head& head, const unsigned char str, char* buf, int seq)
{
    ...
    head.seq = u_short(seq); //序列号与当前序列号相同
...
int main() {
    ...
    while (1) {
        ...
        cout << "【文件传输成功】" << endl;
        cout << "【文件名称】=" << file << endl;
        cout << "【文件大小】 =" << ((double)datalength) / 1024 << "KB" << endl;
        cout << "【传输总时间】=" << end - starttime << "ms" << endl;
        cout << "【吞吐率】 =" << ((double)datalength) / (double(end - starttime)) << "byte/ms" << endl;
        cout << "---------------------------------------------------" << endl;
        delete[] name;
        delete[] data;
        break;
